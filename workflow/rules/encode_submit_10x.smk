...
rule prepare_star_solo_10x_submission_metadata:
    input:
        ...
        gene_unique_filtered = "{}_Unique_filtered.tar.gz".format(get_gene_model(config)),
        gene_unique_filtered_md5 = "{}_Unique_filtered.tar.gz.md5".format(get_gene_model(config)),
        gene_multi_filtered = "{}_EM_filtered.tar.gz".format(get_gene_model(config)),
        gene_multi_filtered_md5 = "{}_EM_filtered.tar.gz.md5".format(get_gene_model(config)),
        gene_unique_raw = "{}_Unique_raw.tar.gz".format(get_gene_model(config)),
        gene_unique_raw_md5 = "{}_Unique_raw.tar.gz.md5".format(get_gene_model(config)),
        gene_multi_raw = "{}_EM_raw.tar.gz".format(get_gene_model(config)),
        gene_multi_raw_md5 = "{}_EM_raw.tar.gz.md5".format(get_gene_model(config)),
...
rule submit_processed_data:
    ...
    output:
        ...
        gene_unique_filtered = "{}_Unique_filtered.tar.gz.{}.upload".format(get_gene_model(config), get_submit_host(config)),
        gene_multi_filtered = "{}_EM_filtered.tar.gz.{}.upload".format(get_gene_model(config), get_submit_host(config)),
        gene_unique_raw = "{}_Unique_raw.tar.gz.{}.upload".format(get_gene_model(config), get_submit_host(config)),
        gene_multi_raw = "{}_EM_raw.tar.gz.{}.upload".format(get_gene_model(config), get_submit_host(config)),
