...
def load_experiments(experiment_filenames, sep='\t', analysis_root=None):
    ...
    for experiment_filename in experiment_filenames:
        ...
        table = pandas.read_csv(
            experiment_filename, sep=sep, comment='#',
...
def load_library_tables(table_filenames, sep='\t', analysis_root=None):
    ...
    for library_file in table_filenames:
        ...
        table['analysis_dir'] = table['analysis_dir'].apply(os.path.expanduser)
...
def load_gene_coverage(filename, library_id, gene_normalization=None):
...
def load_all_gene_coverage(libraries, gene_list=[], gene_normalization=None):
    if len(libraries) > 0:
