...
def loadAnnotation(filename, source_type, gene_type_filter):
    if filename.endswith(".h5"):
        return load_gtf_cache(filename, source_type, gene_type_filter)
    else:
...
def cache_query_validate(name, value):
    if value is None:
        return
    assert isinstance(value, str), "{} needs to be a string".format(name)
    for badchar in "\\\"":
        assert badchar not in value, "Found invalid character {} in field {}".format(
            badchar, name)


def load_gtf_cache(filename, source_type=None, gene_type_filter=None):
    import pandas

    condition = ["type==\"exon\""]
    cache_query_validate("source_type", source_type)
    if source_type is not None:
        condition.append("and source==\"{}\"".format(source_type))
    cache_query_validate("gene_type_filter", gene_type_filter)
    if gene_type_filter is not None:
        condition.append("and gene_type==\"{}\"".format(gene_type_filter))

    store = pandas.HDFStore(filename, "r")
    cols = ["gene_id", "transcript_id", "chromosome", "start", "stop", "strand"]
    table = store.select("/gtf", where=" ".join(condition), columns=cols)
    geneDict = {}
    strand_decode = {
        -1: '-',
        0: '.',
    ...
    for i, row in table.iterrows():
        geneDict.setdefault(row.gene_id, {}).setdefault(row.transcript_id, []).append(
            (row.chromosome, row.start, row.stop, strand_decode[row.strand]))
    store.close()
    return geneDict
